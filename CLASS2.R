...
# Printing "Hello R" using for loop
for (i in 1:5) {
  print("Hello R")
}

# Printing numbers from 5 to 10
for (m in 5:10)
  print(m)
...
myThirdVector <- c(TRUE, FALSE, TRUE)
myThirdVector
...
# Compute element-wise sum of vectors X and Y (with different lengths)
X <- c(1, 2, 3)
Y <- c(4, 5, 6, 7, 8, 9)
Z <- X + Y
X
...
# Printing which numbers in Tax are less than 50
for (i in 1:6)
  if (Tax[i] < 50)
    print(i)
